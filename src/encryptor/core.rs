...
pub fn encrypt_chunk<C: Crypto>(plain: &Vec<u8>, key: &TKey<C>, nonce: &TNonce<C>) -> Result<Vec<u8>> {
...
pub fn decrypt_chunk<C: Crypto>(encrypted: &Vec<u8>, key: &TKey<C>, nonce: &TNonce<C>) -> Result<Vec<u8>> {
    let cipher = C::new(&key);
